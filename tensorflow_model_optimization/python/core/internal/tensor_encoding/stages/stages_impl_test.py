...
class RotationAwareSignEncodingStageStageTest(test_utils.BaseEncodingStageTest):
  ...
  def test_scaled_sign(self):
    ...
    self.assertAllClose(
      tf.sign(test_data.x) * test_data.encoded_x[stages_impl.RotationAwareSignEncodingStage.SCALE_VALUES_KEY],
      test_data.decoded_x)
  ...
  def test_one_bit_encoding(self):
    ...
    bits = test_data.encoded_x[stages_impl.RotationAwareSignEncodingStage.ENCODED_VALUES_KEY]
    one_bit_packing_stage = stages_impl.BitpackingEncodingStage(1)
    packing_test_data = self.run_one_to_many_encode_decode(one_bit_packing_stage, lambda: bits)
