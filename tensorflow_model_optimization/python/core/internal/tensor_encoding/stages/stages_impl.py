...
class RotationAwareSignEncodingStage(encoding_stage.EncodingStageInterface):
  ...
  The encoding stage encodes vectors into a single bit per coordinate. It is designed
  to execute after a random rotation (such as the result of the `HadamardEncodingStage`).
  It calculates an appropriate scale that ensures that the dequantized tensor
  will be: (a) unbiased, or (b) with a minimal MSE after the inverse rotation.
  """
